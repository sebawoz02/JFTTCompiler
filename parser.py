...
class Parser(SlyPar):
    ...
    def command(self, p) -> int:  # return number of lines of code written
        if self.pg.definition:
            self.pg.insert_fixup_info(
                self.pg.get_curr_step_idx() - p[1] - p[3][0], "REPEAT_BEGINS"
            )
    ...
    def proc_head(self, p) -> None:
        self.EXCEPTION_WRAPPER(
            NameError, self.pg.declare_new_procedure, self.allocator, p[0], p[2]
        )
    ...
    def proc_call(self, p) -> int:
        if p[0] not in self.pg.procedures_dict.keys():
            print(
                f"\033[91mUse of undeclared procedure '{p[0]}' in line {p.lineno}!\033[0m"
            )
            self.cg.close()
            exit(1)
        if self.pg.definition:
            if p[0] == self.pg.current_procedure_name:
                print(
                    f"\033[91mError at line {p.lineno} in procedure '{p[0]}': Recursion is prohibited!\033[0m"
                )
                self.cg.close()
                exit(1)
        ...
        return self.EXCEPTION_WRAPPER(
            NameError, self.pg.generate_procedure, self.cg, p[0], p[2]
        )
    ...
    def declarations(self, p) -> None:
        if self.pg.definition:
            ...
            return
        self.EXCEPTION_WRAPPER(
            NameError, self.allocator.allocate, int(p.NUM), p.PIDENTIFIER
        )
    ...
    def args(self, p) -> list[str]:
        self.check_if_set(
            ValInfo(p[2], "PIDENTIFIER", ident=p[2]), p, mode="procedure call"
        )
        if self.pg.definition:
            self.pg.set_variable(ValInfo(p[2], "PIDENTIFIER", ident=p[2]))
            return [
                *p[0],
                self.EXCEPTION_WRAPPER(NameError, self.pg.get_param_info, p[2]),
            ]
    ...
    def args(self, p) -> list[str]:
        self.check_if_set(
            ValInfo(p[0], "PIDENTIFIER", ident=p[0]), p, mode="procedure call"
        )
        if self.pg.definition:
    ...
    def expression(self, p) -> ValInfo | int:
        ...
        if self.pg.definition:
            ...
            return ValInfo(
                None, "EXPRESSION", self.pg.add_func_with_two_values(self.cg.add_sub, p)
            )
    ...
    def expression(self, p) -> ValInfo | int:
        ...
        if self.pg.definition:
            ...
            self.pg.add_step(
                self.cg.add_sub,
                [
                    ValInfo(p[0].value[0], p[0].v_type),
                    ValInfo(p[2].value[0], p[2].v_type, 0),
                    "sub",
                ],
                opt,
            )
            return ValInfo(None, "EXPRESSION", 1)
        return self.cg.add_sub(
            p[0], p[2], mode="sub"
    ...
    def expression(self, p) -> ValInfo | int:
        ...
        if self.pg.definition:
            ...
            return ValInfo(
                None,
                "EXPRESSION",
                self.pg.add_func_with_two_values(self.cg.multiply, p),
            )
    ...
    def expression(self, p) -> ValInfo | int:
        ...
        if self.pg.definition:
            if p[0].v_type == "NUM" and p[2].v_type == "NUM":
                ...
                return ValInfo((p[0].value[0] / p[2].value[0], None), "NUM")
            return ValInfo(
                None, "EXPRESSION", self.pg.add_func_with_two_values(self.cg.divide, p)
            )
    ...
    def expression(self, p) -> ValInfo | int:
        ...
        if self.pg.definition:
            ...
            self.pg.add_step(
                ...
                [
                    ValInfo(p[0].value[0], p[0].v_type),
                    ValInfo(p[2].value[0], p[2].v_type),
                    "modulo",
                ],
                opt,
            )
            return ValInfo(None, "EXPRESSION", 1)
        return self.cg.divide(
            p[0], p[2], mode="modulo"
        )  # number of lines of code written
    ...
    def condition(self, p):
        ...
        if self.pg.definition:
            ...
            return (
                r,
                self.pg.get_curr_step_idx(),
            )  # return current step idx to make jump fix easier
    ...
    def identifier(self, p):
        ...
        return ValInfo(
            self.EXCEPTION_WRAPPER(NameError, self.allocator.get_index, p.PIDENTIFIER),
            "PIDENTIFIER",
            ident=p[0],
        )
    ...
    def identifier(self, p):
        ...
        return ValInfo(
            [
                self.EXCEPTION_WRAPPER(NameError, self.allocator.get_index, p[0]),
    ...
    def error(self, p):
        self.cg.close()
        print(
            f"\033[91mSyntax error at line {p.lineno}: Unexpected token '{p.type}'\033[0m"
        )
