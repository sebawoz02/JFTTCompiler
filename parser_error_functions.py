...
def check_if_set(par, pp, p, warnings, mode="error"):
    def error_msg():
        if mode == "error":
            _exit(
                par,
                f"\033[91mUse of unset variable '{pp.identifier}' in line {p.lineno}!\033[0m",
            )
    ...
    if par.pg.definition:
        ...
        if not par.allocator.is_set(pp.identifier[0]) and par.allocator.is_set(
            pp.identifier[1]
        ):
            error_msg()
...
def check_if_not_array(par, pp, p):
    if par.pg.definition:
        ...
        if info["type"] == "T":
            _exit(
                par,
                f"\033[91mProhibited use of array type variable '{pp}' in line {p.lineno}!\033[0m",
            )
    else:
...
def check_if_array(par, pp, p):
    if par.pg.definition:
        ...
        if info["type"] == "1":
            _exit(
                par,
                f"\033[91mProhibited use of int type variable '{pp}' in line {p.lineno}!\033[0m",
            )
    else:
        info = par.EXCEPTION_WRAPPER(NameError, par.allocator.get_type, pp)
        if info == "1":
...
def check_if_index_in_range(par, pp, num, p):
    if par.pg.definition:
        ...
        if info["size"] <= num and info["size"] != -1:
            _exit(
                par,
                f"\033[91mArray '{pp}' index {num} out of range! Line {p.lineno}.\033[0m",
            )
    else:
